...
bool Triangle::Read(std::istream& is, const char *txt) {
    bool readA = a_.Read(is, "Введите точку A в виде (x,y) ");
    if (!readA) {
        return false;
    }
    
    bool readB = a_.Read(is, "Введите точку B в виде (x,y) ");
    if (!readB) {
        return false;
    }
    
    bool readC = a_.Read(is, "Введите точку C в виде (x,y) ");
    if (!readC) {
        return false;
    }
...
//	Line c(b_, c_);
    ...
    return 0; // c.length() + b.length() + b.length();
}
